...
//	if(enInterleaveMode==1)
	...
	for(i=0;i<segmentNum;i++)
	{
		...
		sprintf(cmdBuf,"cat %s >> %s", nameBuf,outFileName );
		DEBUG("cmdBuf=%s\n",cmdBuf);
		system(cmdBuf);
...
int main(char argc, char *argv[])
{
	...
	if(enPartialAuth==0)
	{
		ret=_genSubSecureInfo(UTILITY_PATH,OUTPUT_FILE_NAME,INPUT_FILE_NAME,1,PRIVATE_KEY,PUBLIC_KEY,0);
		if((enInterleaveMode==1)&(ret!=-1))
		{
			memset(tempBuf,0,sizeof(tempBuf));
			sprintf(tempBuf,  "%s.sig", INPUT_FILE_NAME);
			DEBUG("tempBuf=%s\n",tempBuf);
			ret=_genSubSecureInfo(UTILITY_PATH,tempBuf,INPUT_FILE_NAME,1,PRIVATE_KEY,PUBLIC_KEY,1);
		}
		else
		{
			ERROR("create sub secure info for non-interleave mode fail\n");
			return -1;
		}
		if(ret==-1)
		{
		...
		memset(cmdBuf,0,sizeof(cmdBuf));
		sprintf(cmdBuf,"cat %s >> %s", tempBuf,OUTPUT_FILE_NAME );
		DEBUG("cmdBuf=%s\n",cmdBuf);
		system(cmdBuf);
	...
	{
		ret=genSecureInfo(INPUT_FILE_NAME, OUTPUT_FILE_NAME,segmentNum, UTILITY_PATH, PRIVATE_KEY, PUBLIC_KEY, 0, blockSize);
		if((enInterleaveMode==1)&(ret!=-1))
		{
			ret=genSecureInfo(INPUT_FILE_NAME, OUTPUT_FILE_NAME,segmentNum, UTILITY_PATH, PRIVATE_KEY, PUBLIC_KEY, 1, blockSize);
		}
	}
