    ...
    if (NULL == t_fpin)
    {
        ...
        else
        {
    	  ...
    	  fseek(t_fpin, 0, SEEK_SET);
			  ...
			  t_buf = (unsigned char*)malloc(t_filesize + iAlignment);
    	  if (NULL == t_buf)
    	  {
        ...
        {
        		unsigned char patch = t_filesize%iAlignment;
        		unsigned char i;
            fread(t_buf, 1, t_filesize, t_fpin);
            fclose(t_fpin);
            if(patch != 0)
            {
                for(i=0;i<(iAlignment-patch);i++)
                {
                    t_buf[t_filesize + i] = 0xFF;	
                }
                t_filesize = t_filesize + (iAlignment - patch);
            }	
        }
    }
